...
def _assert_market_or_slot_open(db: Session, market: Market, slot_id: int | None) -> None:
...
def submit_bulk_simulation(
    ...
    *,
    ...
    _assert_market_or_slot_open(db, market, slot_id)
    ...
    try:
        credit_service.apply_ledger_entry(
            db, user=user, type="stake", amount=-total_credits,
            reference_type="simulation_batch", reference_id=str(batch.id),
            created_by_admin_id=admin_id, note=f"{len(entries)} {game_type.code} selections on {market.name}",
        )
