...
async def grant(
    ...
    credit_service.grant_credits(db, user, payload.amount, current_admin.id, payload.note)
...
async def adjust(
    ...
    user = _get_user(db, user_id)
    try:
        credit_service.adjust_credits(db, user, payload.amount, current_admin.id, payload.note)
...
async def reset(
    ...
    user = _get_user(db, user_id)
    credit_service.reset_credits(db, user, payload.new_balance, current_admin.id, payload.note)
...
async def history(
    ...
    items = [
        CreditLedgerOut(
            ...
            referenceType=r.reference_type, referenceId=r.reference_id, note=r.note, createdAt=r.created_at,
        )
        for r in rows
    ]
    return Page(items=items, total=total, limit=pagination.limit, offset=pagination.offset)
