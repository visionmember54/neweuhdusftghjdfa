...
async def simulation_statistics(current_admin: Admin = Depends(get_current_admin), db: Session = Depends(get_db)):
    return compute_market_statistics(db)


@router.get("/game-statistics")
async def game_statistics(current_admin: Admin = Depends(get_current_admin), db: Session = Depends(get_db)):
