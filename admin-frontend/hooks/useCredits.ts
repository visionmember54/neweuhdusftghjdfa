...
export function useGrantCredits() {
  ...
  return useMutation({
    mutationFn: ({ studentId, amount, note }: { studentId: number; amount: number; note: string }) =>
      api.post<Student>(`/admin/users/${studentId}/credits/grant`, { amount, note }),
...
export function useAdjustCredits() {
  ...
  return useMutation({
    mutationFn: ({ studentId, amount, note }: { studentId: number; amount: number; note: string }) =>
      api.post<Student>(`/admin/users/${studentId}/credits/adjust`, { amount, note }),
    onSuccess: () => invalidateAfterCreditChange(qc),
  });
}
...
export function useResetCredits() {
  const qc = useQueryClient();
  return useMutation({
    mutationFn: ({ studentId, new_balance, note }: { studentId: number; new_balance: number; note: string }) =>
      api.post<Student>(`/admin/users/${studentId}/credits/reset`, { new_balance, note }),
    onSuccess: () => invalidateAfterCreditChange(qc),
  });
}
