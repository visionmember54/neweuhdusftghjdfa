...
function LearningCreditsPageInner() {
  ...
  const selectedStudent = studentsPage?.items.find((s) => s.id === studentId);
      ...
      <CardBody>
        <form
          onSubmit={async (e) => {
            e.preventDefault();
            if (!note.trim()) return;
            await grant.mutateAsync({ studentId, amount: Number(amount), note: note.trim() });
            setAmount('');
            setNote('');
          }}
        >
      ...
      <CardBody>
        <form
          onSubmit={async (e) => {
            e.preventDefault();
            if (!note.trim()) return;
            await adjust.mutateAsync({ studentId, amount: Number(amount), note: note.trim() });
            setAmount('');
            setNote('');
          }}
        >
      ...
      <CardBody>
        <form
          onSubmit={async (e) => {
            e.preventDefault();
            if (!note.trim()) return;
            await reset.mutateAsync({ studentId, new_balance: Number(newBalance), note: note.trim() });
            setNote('');
          }}
        >
