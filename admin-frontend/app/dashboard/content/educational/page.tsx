        ...
        {content?.map((c) => (
          <Card key={c.id}>
            <div className="p-5">
              ...
              <div className="mt-4 flex gap-3"><button className="text-xs font-semibold text-brand-600 hover:underline" onClick={() => setEditing(c)}>Edit</button><button className="text-xs font-semibold text-red-600 hover:underline" onClick={() => { if (window.confirm(`Delete ${c.title}?`)) deleteContent.mutate(c.id); }}>Delete</button></div>
            </div>
          </Card>
        ))}
      </div>
